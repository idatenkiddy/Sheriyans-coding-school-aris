...
function showTheCards(filteredArr) {
  var htmladder = "";
...
function handleSearchFunctionality() {
  ...
  input.addEventListener("input", function () {
    ...
    const filtered_imgarr = imgarr.filter((obj) =>
      obj.name.toLowerCase().startsWith(searchValue)
    );

    var clutter = "";

    filtered_imgarr.forEach(function (obj) {
    ...
    if (filtered_imgarr.length === 0) {
      clutter = `<div class="res flex px-8 py-3">
      <i class="ri-search-line font-semibold mr-5"></i>
      <h3 class="font-semibold">Not Found 404</h3>
      </div>`;
    }
